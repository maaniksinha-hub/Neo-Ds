...
export function PageErrorState({ illustration, title, message, retryLabel = 'Retry', onRetry, className }: PageErrorStateProps) {
  return (
    <div className={['ds-pageerrorstate', className].filter(Boolean).join(' ')}>
      ...
      {onRetry && (
        <Button variant="outline" type="primary" size="44px" onClick={onRetry}>
          {retryLabel}
        </Button>
      )}
    </div>

...
import { GSolidButton } from './gSolidButton';

const meta = {
  title: 'Components/gSolidButton',
  component: GSolidButton,
  tags: ['autodocs'],
  argTypes: {
...
} satisfies Meta<typeof GSolidButton>;
  ...
  render: () => (
    <div style={{ display: 'flex', gap: 12 }}>
      {(['primary', 'positive', 'negative', 'sell-cta'] as const).map((type) => (
        <GSolidButton key={type} type={type}>
          {type}
        </GSolidButton>
      ))}
    </div>
  ...
  render: () => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      {(['32px', '36px', '44px', '52px'] as const).map((size) => (
        <GSolidButton key={size} size={size}>
          {size}
        </GSolidButton>
      ))}
    </div>
